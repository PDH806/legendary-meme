...
$get_mb_name = trim($_POST['mb_name']);
...
if (empty($mb['mb_id']) || $mb['mb_leave_date']) {
	...
	$sql_name = " select exists (select mb_id from {$g5['member_table']} where mb_name = '$get_mb_name' and mb_hp = '$hp' limit 1) as CHK_EXIST ";
	$row_name = sql_fetch($sql_name);
	if ($row_name['CHK_EXIST'] < 1)
		alert('해당 회원명과  핸드폰번호의 가입자는 없습니다.\\n\\n사무국으로 문의하여 주십시오.');
}
...
// alert_close($email.' 메일로 회원아이디와 비밀번호를 인증할 수 있는 메일이 발송 되었습니다.\\n\\n메일을 확인하여 주십시오.');
	...
	// SMS회신번호가 없을경우
	if ($send_numbers == "") {
		alert('SMS등록이 되지 않아 이메일로 이용해 주십시요.(SMS기본설정>회신번호등록필요/관리자문의)', $href_G5); // 메인페이지로 이동
	}
