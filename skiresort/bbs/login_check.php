...
//비밀번호 변환 SBAK 기존방식 시작=> SHA256   2025년9월 9일 윈츠

$old_password = $mb['mb_password']; //현재 mb_password 값 추출
$chk_password = substr($mb['mb_password'], 0, 6);

if ($mb['mb_password'] <> '' && $chk_password == '$2y$10') { //비번이 있으면서, 옛날 알고리즘으로 시작하면

    if (!$is_need_not_password && (! (isset($mb['mb_id']) && $mb['mb_id']))) {
        run_event('password_is_wrong', 'login', $mb);
        alert('가입된 회원아이디가 아니거나 비밀번호가 틀립니다.\\n비밀번호는 대소문자를 구분합니다.');
    }

    if (password_verify($mb_password, $old_password)) { //비번이 일치하면
        $sha_password = get_encrypt_string($mb_password); //sha256 암호화 변환
        $sql = "update {$g5['member_table']} set mb_password = '$sha_password' where mb_id = '{$mb['mb_id']}' ";
        sql_query($sql);
    } else { // 불일치하면
        run_event('password_is_wrong', 'login', $mb);
        alert('가입된 회원아이디가 아니거나 비밀번호가 틀립니다.');
    }
} else {  // 아래 부분이 그누보드의 원본 코드
...
// 로그인하면 무조건 마이페이지로 가게 하자. 251016 박순진
$link = G5_THEME_URL . "/html/my_form/sbak_console_01.php";
goto_url($link);
