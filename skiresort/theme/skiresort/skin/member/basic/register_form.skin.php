...
<div class="register">
	<form id="fregisterform" name="fregisterform" action="<?php echo $register_action_url ?>" onsubmit="return fregisterform_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off">
		...
		<input type="hidden" name="a_auth" value="<?php echo $a_auth ?>">
		...
		<div id="register_form" class="form_01">
			<div class="register_form_inner">
				...
				<ul>
					...
					<li class="half_input left_input margin_input">
						<label for="reg_mb_password"><b>비밀번호</b> (필수)</label>
					...
					<li class="half_input left_input">
						<label for="reg_mb_password_re"><b>비밀번호 확인</b> (필수)</label>
			...
			<div class="tbl_frm01 tbl_wrap register_form_inner">
				<h2>개인정보 입력 (<font color=red> 이름, 생년월일, 성별은 자격증과 연동되오니 정확히 입력하세요. 이후 수정불가합니다.</font>)</h2>
				<ul>
					<li>
						<?php
						$desc_name = '';
						...
						if ($config['cf_cert_use']) {
							...
							// echo '<span class="cert_req">(필수)</span>'; //대현
					...
					<li>
						<label for="reg_mb_name"><b>이름</b> (필수 / 띄워쓰기 하지마세요.)<?php echo $desc_name ?></label>
					...
					<li>
						<label for="reg_mb_email"><b>E-mail</b> (필수)
					...
					<!--250919 대현--------------------------------------------------------------------------------------------------->
					<li class="half_input left_input">
						<label for="reg_mb_gender"><b>성별</b> (필수)</label>
						남자 <input type="radio" name="mb_1" required value="남자" id="reg_mb_1" <?php if (get_text($member['mb_1']) == '남자')
																									echo "checked"; ?>>
						여자 <input type="radio" name="mb_1" value="여자" id="reg_mb_11" <?php if (get_text($member['mb_1']) == '여자')
																							echo "checked"; ?>>
					</li>

					<li class="half_input left_input margin_input">
						<label for="reg_mb_homepage"><b>생년월일</b> (필수)</label>
						<input type="date" name="mb_2" value="<?php echo get_text($member['mb_2']) ?>" id="reg_mb_2"
							class="frm_input full_input required" width="350px" placeholder="생년월일" <?php if ($is_member)
																										echo "readonly"; ?>>
						<div id="result_age" style="color:red"></div>
					</li>
					...
					<li class="half_input left_input margin_input">
						<?php if ($config['cf_use_hp'] || ($config["cf_cert_use"] && ($config['cf_cert_hp'] || $config['cf_cert_simple']))) {  ?>
							<label for="reg_mb_hp"><b>휴대폰번호</b><?php if (!empty($hp_required)) { ?> (필수/숫자로만 입력하세요.)<?php } ?><?php echo $desc_phone ?></label>

							<input type="text" name="mb_hp" value="<?php echo get_text($member['mb_hp']) ?>" id="reg_mb_hp" <?php echo $hp_required; ?> <?php echo $hp_readonly; ?> class="frm_input full_input <?php echo $hp_required; ?> <?php echo $hp_readonly; ?>" maxlength="20" placeholder="휴대폰번호" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" />
					...
					<li class="half_input left_input">
						...
						<?php //휴대폰 간편인증때문에 삽입한 부분, 박순진
						if ($w == '' && $config['cf_auth_hp']) {
						?>

							<button type="button" id="win_hp_auth" style="width:30%;float:right" class="btn_frmline"
								onclick="getCode(this)">인증번호받기</button>
							<div style="display:none;padding-top:40px" id="display_hp_certi">
								<input type="number" name="auth_num" id="auth_num" class="frm_input" style="float:left"
									placeholder="인증번호 5자리 입력" maxlength="5">

								<input type="button" name="btnPhone" class="frm_btn2" value="인증확인"
									style="width:30%;float:right" onclick="authCheck();">
							</div>
			...
			<div class="tbl_frm01 tbl_wrap register_form_inner">
				<h2>기타 개인설정 (<font color=red> 회원사진 등록은 회원가입을 완료한 이후 회원정보수정에서 등록가능합니다.</font>)</h2><br>
				<?php
				if ($is_member) {
					echo "(회원사진은 각종 검정 신청 및 진행시 본인확인을 위한 사진대조, 자격증제작 등에 사용되오니, 반드시 상반신만 크게 나온
				신분증 형태의 사진을 등록해주세요. 사진 가로 세로 비율은 3대4 정도, 파일종류는 jpg, png, gif만 가능하고, 파일크기는 1.5MB까지 가능합니다.)
			    <br> * 부적절 사진  : 단체사진, 전신사진, 마스크착용사진, 선글라스/고글착용사진, 흐린사진, 합성사진 등 (운전면허증, 여권에 준하는 형태의 사진만 가능 )";
				}
				?>
				...
				<ul>
					...
					<?php if ($config['cf_use_profile']) {  ?>
						<li>
							<label for="reg_mb_profile"><b>자기소개</b> (기술선수권대회 등 각종 행사시 사용될 본인 소개 멘트)</label>
...
<script>
	<?php if ($w == '' && $config['cf_auth_hp']) { ?>
		...
		function checkPhone() {
			...
			var the_phone = document.getElementById('reg_mb_hp').value;
			var the_name = document.getElementById('reg_mb_name').value;
			var the_nick = document.getElementById('reg_mb_nick').value;
			var the_birth = document.getElementById('reg_mb_2').value;
			var the_email = document.getElementById('reg_mb_email').value;
			var the_id = document.getElementById('reg_mb_id').value;
			...
			if (the_nick.length < 3) {
				alert("닉네임을 적절히 입력해주세요");
				return false;
			}

			if (the_id.length < 3) {
				alert("아이디를 적절히 입력해주세요");
				return false;
			}

			if (the_birth.length < 3) {
				alert("생년월일을 정확히 선택해주세요");
				return false;
			}

			if (the_email.length < 3) {
				alert("이메일을 정확히 입력해주세요");
				return false;
			}

			if (the_phone.length < 11) {
			...
			document.getElementById("display_hp_certi").style.display = 'block';
			return true;
		}
		var countdown = 60;
				...
				if (f1.auth_num.value == "") {
					...
					$.post("/bbs/ajax_check.php", $("#fregisterform").serialize(), function(data) {
						if (data == "Y") {
							...
							nquanju = true;
							document.getElementsByName("btnPhone")[0].value = "인증완료";
							document.getElementsByName("btnPhone")[0].disabled = true;
							document.getElementsByName("auth_num")[0].readonly = true;
							document.getElementById("win_hp_auth").disabled = true;
							alert("정상적으로 인증되었습니다.");
...
<script>
	//14세 체크하기
	// reg_mb_2.onchange = function() {
	// 	var birthDate = document.getElementById("reg_mb_2").value;
	// 	var birthDate = birthDate.replace(/-/g, '');

	// 	var today = new Date();
	// 	var yyyy = today.getFullYear();
	// 	var mm = today.getMonth() < 9 ? "0" + (today.getMonth() + 1) : (today.getMonth() + 1); // getMonth()
	// 	var dd = today.getDate() < 10 ? "0" + today.getDate() : today.getDate();
	// 	var todays = parseInt(yyyy + mm + dd);

	// 	var n = todays - parseInt(birthDate) - 140000;
	// 	if (n < 0) {
	// 		document.getElementById('result_age').innerText = "만 14세 미만이므로 법률에 따라, 법정대리인의 동의가 필요합니다. 현재 법정대리인 연락처를 통한 확인 후 정상처리되며, 이후 법정대리인 본인인증강화예정입니다.";
	// 		return;
	// 	} else {
	// 		document.getElementById('result_age').innerText = "만 14세 이상입니다.";
	// 		return;
	...
	//251027대현 14세 체크 수정
	reg_mb_2.onchange = function() {
    var birthDateStr = document.getElementById("reg_mb_2").value;
    ...
    // 만 14세가 되는 날 계산
    var fourteenYearsLater = new Date(birthDate.getFullYear() + 14, birthDate.getMonth(), birthDate.getDate());

    if (today < fourteenYearsLater) {
        document.getElementById('result_age').innerText =
            "만 14세 미만이므로 법률에 따라, 법정대리인의 동의가 필요합니다. 현재 법정대리인 연락처를 통한 확인 후 정상처리되며, 이후 법정대리인 본인인증강화예정입니다.";
    } else {
        document.getElementById('result_age').innerText = "만 14세 이상입니다.";
    }
};
