...
for ($i = 1; $i <= $data->sheets[0]['numRows']; $i++) {
	    ...
	    */
	    ...
	    $sql = "select count(*) cnt from t_reserve where bib_num='{$data->sheets[0]['cells'][$i][3]}' and gd_seq='GD4503'";
	    $r = sql_fetch($sql);
	    if($r['cnt']=='1'){
	     echo $data->sheets[0]['cells'][$i][1].'->'.$data->sheets[0]['cells'][$i][2].'->'.$data->sheets[0]['cells'][$i][3].'->'.$data->sheets[0]['cells'][$i][6].'<br>';
	     /*$sql = "update t_reserve set license_num='{$data->sheets[0]['cells'][$i][2]}' where bib_num='{$data->sheets[0]['cells'][$i][3]}' and gd_seq='GD4503'";
	     sql_query($sql,true);
	     */
	    }
