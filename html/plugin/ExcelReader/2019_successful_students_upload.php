...
$data->read('./data/2020_skiT2_success.xls');
...
for ($i = 1; $i <= $data->sheets[0]['numRows']; $i++) {
    ...
    $sql = "select count(*) cnt from t_reserve where license_num = '{$data->sheets[0]['cells'][$i][2]}' and gd_seq='gd4505' ";
    $cnt = sql_fetch($sql);
    if(!$cnt[cnt]) echo $sql.'<br>';
}
exit;
*/
...
for ($i = 1; $i <= $data->sheets[0]['numRows']; $i++) {
	    ...
	    */
	    // 스키티칭2 : 500053 / GD4505
	    $sql = "update t_reserve set license_num='{$data->sheets[0]['cells'][$i][2]}' 
where idkey='500053' and gd_seq='gd4505' 
and rs_buyer_name='{$data->sheets[0]['cells'][$i][3]}' 
and rs_buyer_tel='{$data->sheets[0]['cells'][$i][5]}' 
and bib_num='{$data->sheets[0]['cells'][$i][1]}' 
";
	    //echo $sql.'<br>';
