...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    if ($start_page > 1) $str .= '<li class="prev"><a href="'.$url.($start_page-1).$add.'" class=""><img src="/ski/images/prev_icon.gif" alt="이전"></a></li>'.PHP_EOL;

    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= '<li><a href="'.$url.$k.$add.'" class="">'.$k.'<span class="sound_only">페이지</span></a></li>'.PHP_EOL;
            else
                $str .= '<li class="on"><a href="javascript:void(0);">'.$k.'</a></li>'.PHP_EOL;
        }
    }

    if ($total_page > $end_page) $str .= '<li class="next"><a href="'.$url.($end_page+1).$add.'" ><img src="/ski/images/next_icon.gif" alt="다음"></a></li>'.PHP_EOL;
    
    //$str .= '<li class="next"><a href="'.$url.($end_page+1).$add.'" ><img src="/ski/images/next_icon.gif" alt="다음"></a></li>'.PHP_EOL;
    ...
    if ($str)
        return "<div class='prev_next_box'><ul style='text-align:center'>{$str}</ul></div>";
    else
        return "";
}
