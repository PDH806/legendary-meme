 ...
 */
 ...
 if(strlen($_POST['birth1'])==4){
     ...
     $tbirth = substr($_POST['birth1'],2,2); // 2자리 년도
 }
 ...
 if($_POST['uploadPhoto']){
     ...
     if($_SERVER['REMOTE_ADDR']=='210.97.98.158'){
         /*echo "update t_reserve set rs_buyer_photo='{$_POST['uploadPhoto']}' ,rs_buyer_photo_old='{$_POST['uploadPhoto']}' where IDKEY='{$_POST['key']}' AND  GD_SEQ='{$_POST['seq']}' 
                AND rs_buyer_name='{$_POST['hname']}' and rs_buyer_idkey='$birth'";
         exit;
         */
     }
 
 sql_query("update t_reserve set rs_buyer_photo='{$_POST['uploadPhoto']}' ,rs_buyer_photo_old='{$_POST['uploadPhoto']}' where IDKEY='{$_POST['key']}' AND  GD_SEQ='{$_POST['seq']}' 
                AND rs_buyer_name='{$_POST['hname']}' and rs_buyer_idkey='$birth'");
 }
 ...
 // 신청정보 업데이트
 sql_query("update t_reserve set  DELIVERY_REQ_YN='Y' where IDKEY='{$_POST['key']}' AND  GD_SEQ='{$_POST['seq']}'
                AND rs_buyer_name='{$_POST['hname']}' and rs_buyer_idkey='$birth'");
 
?>
<?php if($rs){?>
<form name='f1' action='/ski/teach_my_info.html' method='post'>
<?php 
