<?php
 ...
 $rs = sql_query($sql);
  ...
  if(strpos($_POST['key'],',')){
   ...
   $tmp_key = "('".implode("','" , explode(',',$_POST['key']))."')";
   $t_idkey = " IDKEY in {$tmp_key}";
   
  }else{
   $t_idkey = " IDKEY ='{$tmp_key}'";
  }
   ...
   // 업로드한 파일 DB 입력
   if($_POST['uploadPhoto']){
       sql_query("update t_reserve set rs_buyer_photo='{$_POST['uploadPhoto']}' where {$t_idkey} AND  GD_SEQ='{$_POST['seq']}'
                AND rs_buyer_name='{$_POST['hname']}' and rs_buyer_idkey='$birth'");
   }
   $sql = "UPDATE  t_reserve_add_info SET
            ADD_NAME_KOR='{$_POST['hname']}', ADD_NAME_ENG='{$_POST['ename']}', ADD_BIRTH='{$tbirth}', ADD_TEL='{$tel}',  ADD_ADDR_TYPE='{$_POST['part']}', ADD_ZIPCODE='{$_POST['zip']}',
            ADD_ADDR1 = '{$_POST['addr1']}' ,  ADD_ADDR2='{$_POST['addr2']}' , UPDATE_DATE = '".G5_TIME_YMDHIS."'
           WHERE {$t_idkey} and GD_SEQ='{$_POST['seq']}' and  RS_SEQ='{$_POST['seq1']}'
...
?>
<?php if($rs){?>
<form name='f1' action='/ski/patrol_my_info_1.html' method='post'>
