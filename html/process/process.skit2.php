 ...
 */
 ...
 /*스키지도T2 마감일
 $date4 = strtotime(date("20190219"));
 $date3 = strtotime(date("20190220"));
 $date2 = strtotime(date("20190221"));
 //$today = strtotime(date("20190222"));
 */
 if($_SERVER['REMOTE_ADDR']=='125.132.71.189'){
     /*echo '$today : ' . $today.'<br>';
     echo '$date4 : ' . $date4.'<br>';
...
where {$_POST['kind']} = '{$_POST['kind_value']}' and RS_BUYER_PWD = '{$_POST['RS_BUYER_PWD']}' and GD_SEQ='{$_POST['seq']}' order by a.rs_date desc limit 1 ";
 ...
 // 등록
     ...
     if($_POST['TID']){
 $sql = "select  GD_CLOSE_DT from t_goods where gd_seq = '{$_POST['seq']}' ";
 $sd_date = sql_fetch($sql);
 
 $sd_date = substr($sd_date['GD_CLOSE_DT'],0,8);
 ...
 $sql = "INSERT INTO  t_reserve SET
            ...
            RS_BUYER_DOC='$db_ins_file', INSERT_IDKEY='$rs_buyer_idkey', RS_CC_CODE='{$_POST['isChk1']}' 
";
  sql_query($sql);
  //echo $sql;exit;
 }
                                ...
                                <div class="accept_wrap menu">
                                    <div class="tab_menu_area inner_menu">
                                        ...
                                        <? if(/*1==1*/$expiry && $r['RS_STATUS'] != 'D'){?>
                                            <li><a href="javascript:frm2.action='../teach_accept_t2_update.html';frm2.submit();">접수수정</a></li>
                                    ...
                                    <div class="tab1">
                                        <div class="patrol_list">
                                            <div class="content_box">
                                                <div class="txt_box">
                                                    <div class="tit_box"><span class="sb_tit">스키지도요원(티칭2) 신청 결제내역</span></div>
