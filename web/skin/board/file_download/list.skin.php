...
if ($board['bo_use_list_file'] == '0') { 
sql_query(" update `".G5_TABLE_PREFIX."board` SET `bo_table` = '".$bo_table."',`gr_id` = '".$gr_id."',`bo_subject` = '".$board['bo_subject']."',`bo_use_list_file` = 1 WHERE `".G5_TABLE_PREFIX."board`.`bo_table` = '".$bo_table."';");
}
if ($board['bo_download_point'] != '0') { 
sql_query(" update `".G5_TABLE_PREFIX."board` SET `bo_table` = '".$bo_table."',`gr_id` = '".$gr_id."',`bo_subject` = '".$board['bo_subject']."',`bo_download_point` = 0 WHERE `".G5_TABLE_PREFIX."board`.`bo_table` = '".$bo_table."';");
}
?>
<!-- 게시판 목록 시작 { -->
			...
			<?php
				...
				if(isset($thumb['src']) && $thumb['src']) {
					...
					if (isset($youtube_video_id) && $youtube_video_id) {
						$img_content = '<a href="'.$list[$i]['href'].'" class="list_img"><i class="youtube_img"><sup style="background-image:URL(//img.youtube.com/vi/'.$youtube_video_id.'/maxresdefault.jpg);" alt="'.$list[$i]['subject'].'"></sup><sub><i class="imgAr"><img src="'.G5_THEME_URL.'/plugin/websiting/imageSpacer.php?w=150&h=150" alt="youtube image spacer"></i></sub></i></a>
					';
